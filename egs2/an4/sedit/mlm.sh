...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        _mlm_train_dir="${data_feats}/${train_set}"
        _mlm_valid_dir="${data_feats}/${valid_set}"
        log "Stage 11: MLM Training: train_set=${_mlm_train_dir}, valid_set=${_mlm_valid_dir}"
        ...
        log "Generate '${mlm_exp}/run.sh'. You can resume the process from stage 11 using this script"
        mkdir -p "${mlm_exp}"; echo "${run_args} --stage 11 \"\$@\"; exit \$?" > "${mlm_exp}/run.sh"; chmod +x "${mlm_exp}/run.sh"
